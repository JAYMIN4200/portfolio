...
<head>
    ...
    <?php $authFavicon = App\Models\Setting::get('favicon'); ?>
    <?php if($authFavicon): ?>
        <link rel="icon" type="image/png" href="<?php echo e(asset('storage/' . $authFavicon)); ?>">
    <?php else: ?>
    ...
    <div class="w-full max-w-md relative">
        <div class="text-center mb-8">
            <div class="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 backdrop-blur flex items-center justify-center overflow-hidden shadow-xl shadow-indigo-500/20 mx-auto mb-4">
                <?php if($authFavicon): ?>
                    <img src="<?php echo e(asset('storage/' . $authFavicon)); ?>" alt="Favicon" class="w-full h-full object-cover">
                <?php else: ?>
        ...
        <div class="bg-white rounded-2xl shadow-2xl border border-white/40 p-8">
            ...
            <form method="POST" action="<?php echo e(route('login.attempt')); ?>" class="space-y-5" data-validate data-validate-messages='<?php echo json_encode((new \App\Http\Requests\LoginRequest)->messages(), 15, 512) ?>' data-validate-error-class="text-red-500 text-xs mt-1">
                <?php echo csrf_field(); ?>
                <div>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <a href="<?php echo e(route('password.request')); ?>" class="text-sm text-indigo-600 hover:text-indigo-700 font-medium">Forgot password?</a>
                </div>
        ...
        <p class="text-center mt-6 text-sm text-slate-400">
            <a href="<?php echo e(route('home')); ?>" class="text-indigo-400 hover:text-indigo-300 font-medium">&larr; Back to website</a>
        </p>
    </div>
