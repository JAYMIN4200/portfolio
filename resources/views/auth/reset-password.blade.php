...
<body class="bg-slate-950 antialiased min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
    ...
    <div class="w-full max-w-md relative">
        <div class="text-center mb-8">
            ...
            <p class="text-slate-400 mt-1 text-sm">Choose a new password for your account.</p>
        </div>
        ...
        <div class="bg-slate-900/70 backdrop-blur-xl rounded-2xl shadow-2xl shadow-indigo-950/40 border border-white/10 p-8">
            ...
            <form method="POST" action="{{ route('password.update') }}" data-submitting class="space-y-5" data-validate data-validate-messages='@json((new \App\Http\Requests\ResetPasswordRequest)->messages())' data-validate-error-class="text-red-500 text-xs mt-1">
                @csrf
                <div>
                    <label for="password" class="block text-sm font-medium text-slate-300 mb-1.5">New Password <span class="text-red-500">*</span></label>
                    <div class="relative">
                        <input type="password" id="password" name="password" required
                               class="w-full px-4 py-3 pr-12 rounded-lg border border-white/10 bg-slate-950/60 text-white placeholder-slate-500 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-all"
                               placeholder="Min. 8 characters">
                        <button type="button" data-password-toggle data-password-toggle-for="password"
                                class="absolute inset-y-0 right-0 flex items-center pr-3 text-slate-400 hover:text-slate-200 transition-colors" aria-label="Show password">
                ...
                <div>
                    <label for="password_confirmation" class="block text-sm font-medium text-slate-300 mb-1.5">Confirm New Password <span class="text-red-500">*</span></label>
                    <div class="relative">
                        <input type="password" id="password_confirmation" name="password_confirmation" required
                               class="w-full px-4 py-3 pr-12 rounded-lg border border-white/10 bg-slate-950/60 text-white placeholder-slate-500 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-all"
                               placeholder="••••••••">
                        <button type="button" data-password-toggle data-password-toggle-for="password_confirmation"
                                class="absolute inset-y-0 right-0 flex items-center pr-3 text-slate-400 hover:text-slate-200 transition-colors" aria-label="Show password">
                            <svg class="w-5 h-5" data-icon-eye fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            <svg class="w-5 h-5 hidden" data-icon-eye-off fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l18 18"/></svg>
                        </button>
