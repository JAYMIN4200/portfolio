...
@php
    ...
    // A fresh install has no profile row yet, so every access stays
    // null-safe and the hero degrades to placeholder copy instead of a 500.
    $ownerName = $profile?->user?->name ?? 'Developer';
    $avatarUrl = $profile?->avatarUrl() ?? asset('images/default-avatar.svg');
@endphp

<section id="home" class="relative min-h-screen flex items-center overflow-hidden">
    <div class="hero-grid absolute inset-0"></div>
    <div class="hero-orb top-1/4 -left-32 w-96 h-96 bg-indigo-600/20" data-parallax="0.12"></div>
    <div class="hero-orb bottom-1/4 -right-32 w-96 h-96 bg-purple-600/20" data-parallax="-0.09"></div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <div data-reveal="up">
                ...
                <h1 class="text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-tight tracking-tight">
                    Hi, I'm <span class="gradient-text">{{ $ownerName }}</span>
                </h1>
                ...
                <div class="mt-8 flex flex-wrap items-center gap-4">
                    <a href="{{ route('projects.index') }}" data-magnetic="0.18" class="inline-flex items-center gap-2 px-6 py-3 rounded-lg bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-semibold shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/50 transition-shadow">
                        View My Work
            ...
            <div class="hidden lg:flex justify-center" data-reveal="right">
                <div class="relative animate-float">
                    ...
                    <div class="relative w-72 h-72 rounded-full overflow-hidden border-4 border-indigo-500/40 shadow-2xl shadow-indigo-500/30">
                        <img src="{{ $avatarUrl }}" alt="{{ $ownerName }}" class="w-full h-full object-cover">
                    </div>
