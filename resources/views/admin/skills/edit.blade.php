...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.skills.update', $skill) }}" class="w-full" data-submitting data-validate data-validate-messages='@json((new \App\Http\Requests\SkillRequest)->messages())' data-validate-error-class="text-red-500 text-xs mt-1">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-5">
                ...
                <div class="grid grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', $skill->sort_order) }}" min="0" placeholder="0"
                               class="w-full px-3 py-2.5 rounded-lg border border-slate-300 text-slate-800 placeholder-slate-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-all">
                    </div>
                </div>
            </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <span data-submit-label>Update Skill</span>
