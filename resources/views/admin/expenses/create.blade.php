...
@section('content')
    ...
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-slate-800">Add New Expense</h1>
        <p class="text-slate-500 mt-1">Record a business expense.</p>
    </div>

    <form method="POST" action="{{ route('admin.expenses.store') }}" class="w-full" data-submitting data-validate data-validate-messages='@json((new \App\Http\Requests\ExpenseRequest)->messages())' data-validate-error-class="text-red-500 text-xs mt-1">
        @csrf
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-6 mb-6">
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required placeholder="e.g. Domain renewal" class="w-full px-3 py-2.5 rounded-lg border border-slate-300 text-slate-800 placeholder-slate-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-all @error('title') border-red-300 @enderror">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                    <div>
                        ...
                        <input type="number" name="amount" value="{{ old('amount') }}" required step="0.01" min="0.01" placeholder="0.00" class="w-full px-3 py-2.5 rounded-lg border border-slate-300 text-slate-800 placeholder-slate-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-all @error('amount') border-red-300 @enderror">
                    ...
                    <div>
                        ...
                        <select name="category" required class="w-full px-3 py-2.5 rounded-lg border border-slate-300 text-sm text-slate-800 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-all @error('category') border-red-300 @enderror">
                            @foreach ($categories as $category)
                                <option value="{{ $category }}" {{ old('category', 'Other') === $category ? 'selected' : '' }}>{{ $category }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="expense_date" value="{{ old('expense_date') }}" required class="w-full px-3 py-2.5 rounded-lg border border-slate-300 text-sm text-slate-800 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-all @error('expense_date') border-red-300 @enderror">
        ...
        <div class="flex items-center gap-3">
            <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 rounded-lg bg-indigo-600 text-white font-semibold text-sm hover:bg-indigo-700 transition-colors">
                <span data-submit-label>Add Expense</span>
