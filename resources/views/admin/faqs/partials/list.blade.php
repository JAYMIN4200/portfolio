    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            <thead class="bg-slate-50 text-slate-600 uppercase text-xs tracking-wider">
                <tr>
                    <th class="px-5 py-3 text-left font-semibold">Question</th>
                    <th class="px-5 py-3 text-left font-semibold">Answer</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($faqs as $faq)
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-5 py-4 font-medium text-slate-800 max-w-xs">{{ $faq->question }}</td>
                        <td class="px-5 py-4 text-slate-500 max-w-sm truncate">{{ $faq->answer }}</td>
                        ...
                        <td class="px-5 py-4 text-right">
                            <div class="inline-flex items-center gap-1">
                                <a href="{{ route('admin.faqs.edit', $faq) }}" class="p-1.5 rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                <form method="POST" action="{{ route('admin.faqs.destroy', $faq) }}" data-confirm="Delete this FAQ?" class="inline">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <tr><td colspan="5" class="px-5 py-12 text-center text-slate-400">No questions yet. <a href="{{ route('admin.faqs.create') }}" class="text-indigo-600 hover:text-indigo-700 font-medium">Add your first.</a></td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
